...
class ByteBuffer {
public:
    ByteBuffer(uint32_t size = BB_DEFAULT_SIZE, bool network_endian = false);
    ByteBuffer(uint8_t* arr, uint32_t size, bool network_endian = false);
    ~ByteBuffer() = default;
...
private:
    ...
    template<typename T> T read() const {
        ...
        if (network_endian)
        {
          data = evilutopia::util::NetworkToHost(data);
        }
        rpos += sizeof(T);
        return data;
    }
    ...
    template<typename T> void append(T data) {
        ...
        if (network_endian)
        {
          data = evilutopia::util::HostToNetwork(data);
        }
